...
* Regular users - Typical individuals access the cluster
* System users - For use by infrastructure related assets, such as _nodes_
* Service Accounts - Special system users that are associated with Projects/Namespaces, These are typically use to run pods or access the cluster within external systems
...
To demonstrate many of the capabilities around managing Role Based Access Control in OpenShift, a sample application will be used showcase not only applications how applications can be used to communicate with the OpenShift API, but also the levels of permissions that can be granted. 
...
#. Environment to support the implementation of this lab
...
The application is a simple golang based service that communicates with the OpenShift to query various assets. The "_403 Forbidden_" error that is displayed is expected and we will work to resolve these conditions throughout the course of the lab.
...
Once in the project, list the running pods
...
With a basic understanding how applications can query information from the OpenShift API, let's return focus to the example application in the `rbac-lab` namespace. As you can see from the application viewed in the web browser, each of the requests against the API are returning a HTTP 403 error. This error indicates that authentication was successful, however the user does not have the appropriate rights to access the requested service.
...
Now, use User Impersonation capabilities to determine if the _default_ Service Account int the `rbac-lab` namespace can list users
...
As expected, the _default_ Service Account does not have access. You may also notice that we needed to give the full name for the Service Account. In prior commands when creating `RoleBinding's` and `ClusterRoleBinidng's`, we did not need to provide `system:serviceaccount` as it was assumed through the `--serviceaccount` flag.
...
While we could confirm that the application can now query for users, lets use User Impersonation to determine ahead of time whether the default Service Account as the appropriate rights.
