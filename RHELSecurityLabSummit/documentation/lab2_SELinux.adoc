...
Connect to _selinux4_ and switch to Enforcing mode
    ...
    [root@selinux1 selinux_scripts]# ssh root@selinux4.example.com
...
At the end we need to install the _setools-console_ package containing SELinux policy quiery tools which will be used for SELinux Troubleshooting.
	...
	[root@selinux1 selinux_scripts]# ssh root@selinux2.example.com
	[root@selinux2 ~]# yum install setools-console
	[root@selinux2 ~]# exit

	[root@selinux1 selinux_scripts]# ssh root@selinux3.example.com
...
=== Lab 2.2.3 Test configured setup
	...
	[root@selinux1 selinux_scripts]# curl selinux2.example.com
	<h1>Default Document Root</h1>

	[root@selinux1 selinux_scripts]# curl selinux2.example.com:7070
	<h1>Custom Document Root</h1>

	[root@selinux1 selinux_scripts]# curl selinux3.example.com
	<h1>Default Document Root</h1>

	[root@selinux1 selinux_scripts]# curl selinux3.example.com:7070
	<h1>Custom Document Root</h1>
...
List all SELinux configuration changes.
    ...
    [root@selinux1 selinux_scripts]# ssh selinux2.example.com
    [root@selinux2 ~]# semanage export
    [root@selinux2 ~]# exit


    [root@selinux1 selinux_scripts]# ssh selinux3.example.com
...
There is a revert script for the lab environment configuration. This script can be used to start from the scratch with this lab and all actions in the _Configuration_ section need to be executed except the package installation.
...
[source]
[lab-user@workstation-GUID ~]# ssh root@selinux1.example.com
[root@selinux1 ~]# ssh root@selinux2.example.com
