...
=== Prerequisites
. Connect to openscap.example.com as follows:
+
 [lab-user@workstation]$ ssh root@openscap.example.com
...
. Switch to the "openscap.example.com" VM. Let's see the compliance content provided by `scap-security-guide`:
+
...
Putting the machine (change its configuration) into compliance is called *remediation* in the SCAP terminology. Remediation changes the configuration of the machine and it is possible that you will lock yourself out or disable important workloads! Please test before deploying. If you want to test remediations as part of this lab, perform them on the "openscap.example.com" VM to avoid modifying the "workstation.example.com" VM. There are two main approaches to remediating.
