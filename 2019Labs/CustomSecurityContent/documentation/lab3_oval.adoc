...
It is a XML-bassed declarative language that is part of the SCAP standard, and we will focus on it's ability to query a system's state.
...
The `ComplianceAsCode` project uses OVAL to write snippets that are interpreted by an OVAL interpreter (such as the OpenSCAP scanner) to detrmine whether the check defined in the snippet passes.
In this track, we will go through the OVAL snippet of the `accounts_tmout` rule.
We will see how it can get complicated soon, and what can we do about it.
...
Let's examine the bash remediation body - as you know from the introductory lab, you can either
...
The check body looks like this:
...
----
+
Note that the delimiters are different than the `jinja2` website shows - i.e. instead of `{% macro ... %}` on the website, we use the `{{% macro ... %}}` form and so on - there is always one curly brace more than the website documentation shows.
...
The modification should be easy - instead that checking that the timeout value `equals` the threshold, we will use the `less than or equal` check as per the https://oval.mitre.org/language/version5.11/ovaldefinition/documentation/oval-common-schema.html#OperationEnumeration[OVAL specification].
...
This would mean adding `(export[\s]+)?` to the regular expression, but as we don't want that group to be registered (a.k.a. stored in memory, captured), we have to https://oval.mitre.org/language/about/re_support_5.6.html[add some special syntax], so we add `(?:export[\s]+)`, and the section becomes
