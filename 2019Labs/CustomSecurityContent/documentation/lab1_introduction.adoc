...
* The `ComplianceAsCode` repository has already been cloned to the `content` directory.
...
* For the purposes of the lab exercises, the project's source code has been edited.
...
. First, go to the Terminal window of your laptop. SSH into the workstation bastion host as *lab-user*. Then, as *lab-user*, ssh into *workstation.example.com*. This is the VM that we will be doing all of our lab exercises from. Don't forget to replace *GUID* with your lab provided *GUID*!
+
[source, text]
[localhost ~]$ ssh lab-user@workstation-GUID.rhpds.opentlc.com
[lab-user@workstation-GUID ~]$ ssh lab-user@workstation.example.com
[lab-user@workstation ~]$

. Make sure that you are in the good starting directory - `/home/lab-user/labs/lab1_introduction`.
+
[source, text]
[lab-user@workstation ~]$ cd /home/lab-user/labs/lab1_introduction
. From this directory, run `./build_product rhel8` to compile content for Red Hat Enterprise Linux 8.
+
[source, text]
[lab-user@workstation ~]$ cd /home/lab-user/labs/lab1_introduction
[lab-user@workstation ~]$ ./build_product rhel8
+
...
The HTML guide filenames have a `ssg-<product>-guide-<profile>.html` form, so the HTML guide for the RHEL 8 OSPP profile has the `ssg-rhel8-guide-ospp.html` form.

. Now, let's take a look at one of the provided HTML guides in a web browser. Go back to the console of your lab VM by going to the tab that you have opened of the lab environment's power control and consoles page. If you need to log in again as the *Lab User*, the password is *r3dh4t1!*.
+
