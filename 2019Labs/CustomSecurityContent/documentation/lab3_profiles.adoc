...
The basic building block of a security policy is ComplianceAsCode is a rule.
...
We have policies for different products.
...
If you attempt to create a YAML file by hand, the most common mistake tends to be using incorrect indentation.
...
.. `selections`: is a list of rules and variables that consist of the profile.
...
At this point, we can reveal to you that ComplianceAsCode already contains an implementation of this requirement in a form of a rule.
