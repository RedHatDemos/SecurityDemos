...
- open the `linux_os/guide/system/accounts/accounts-session/accounts_tmout/bash/shared.sh` file in the text editor:
+
----
...
Either way, the remediation body looks like this:
...
Note that you can select the text, press `Ctrl+C` to copy it to the clipboard, and then use `Ctrl+Shift+V` to paste it into `nano`.
...
Time to save the OVAL, using the `Ctrl+X`, `Enter` and `y`, rebuild the product and run the tests again:
