...
The `ComplianceAsCode` project features a test suite that is very useful when defining what scenarios the check and the remediation are supposed to handle.
...
Concerning scenarios, consider for example the `accounts_tmout` rule - the two simplest cases are should be handled are the following ones:
