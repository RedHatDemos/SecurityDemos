== Lab 4: Automation with Predictive Analytics
...
==== Lab 4.1 Configure *ic1.summit.example.com* client
Red Hat Insights client has a configuration file (insights-client.conf) which can be modified for your needs. In this lab exercise, we will be configuring both the *ic1.summit.example.com* and *ic4.summit.example.com* clients.
...
. Open up an SSH client and connect to your jump server (workstation-GUID.rhpds.opentlc.com) as *lab-user* with the password *r3dh4t1!* (eg. `ssh lab-user@workstation-GUID.opentlc.com`). Don't forget to replace the *GUID* with your provided *GUID*. Once logged in, become root by typing `sudo -i`.
+
[source]
----
...
----
. Now, change the file permissions to 0660. Note that 0660 file permissions will work on remove.conf and it should inherit the correct selinux types.
+
[source]
----
...
. Data collection can be run with the `insights-client` command. Run `insights-client --help` and take look at the significant amount of options used to tailor the execution of Red Hat Insights based on your environment. These can be used to automate routine data collection through Ansible.
[source]
----
...
==== Lab 4.2 Installing the Red Hat Insights client
. If not already there, Open up an SSH client and connect to your jump server (workstation-GUID.rhpds.opentlc.com) as *lab-user* with the password *r3dh4t1!* (eg. `ssh lab-user@workstation-GUID.opentlc.com`). Don't forget to replace the *GUID* with your provided *GUID*. Once logged in, become root by typing `sudo -i`.
+
[source]
----
[localhost ~]$ ssh lab-user@workstation-GUID.rhpds.opentlc.com
...
Loaded plugins: product-id
----
. Now, register this host to Red Hat Insights with the *--display-name* parameter. This parameter allows you to set the displayed hostname to something other than the default.
+
[source]
----
...
=== Lab 4.3 Manually fixing the payload injection security issue
...
+
image:images/lab4/1-insightsoverview.png[200,200]
image:images/lab4/2-insightsoverview2.png[1000,1000]
...
+
image:images/lab4/3-insightsinventory.png[200,200]
image:images/lab4/4-inventory.png[1000,1000]

. Click on the *ic1.summit.example.com* system. You will see the list of issues affecting it when clicking on the system name.
+
image:images/lab4/5-ic1host.png[1000,1000]
...
+
image:images/lab4/6-CVE-2016-5696.png[1000,1000]
+
...
----
. Next, reload the kernel configuration by running `sysctl -p`.
+
[source]
----
[root@ic1 ~]# sysctl -p
...
. After applying the active mitigation, we want to have the system report any changes. Run the `insights-client` command as root on ic1.summit.example.com:
+
[source]
----
...
+
image:images/lab4/7-payloadinjectionsearch-again.png[1000,1000]

. Congratulations, you’re no longer impacted by the payload injection vulnerability!

=== Lab 4.3 Automatically fixing the payload injection security issue using Ansible Automation
...
+
image:images/lab4/8-planner.png[1000,1000]

. Click on *Create a plan*.
+
image:images/lab4/9-createplan.png[1000,1000]
...
+
image:images/lab4/10-playbookplan.png[1000,1000]
...
+
image:images/lab4/11-setlimit.png[1000,1000]
...
+
image:images/lab4/12-plan.png[1000,1000]
...
. Login with *admin* / *r3dh4t1!* to Ansible Tower at https://tower-GUID.rhpds.opentlc.com replacing *GUID* with your lab's *GUID*.

. Click on *Templates* from the main menu at the left. Find the *Insights Facts Scan* job template and click the *rocket ship* to run this job. This Job template will scan the Ansible facts from the host(s) and gather relevant host information necessary to show the Red Hat Insights data for those hosts in Ansible Tower.
+
image:images/lab4/13-scanjob.png[1000,1000]
...
+
image:images/lab4/14-scanoutput.png[1000,1000]

. Navigate to *Inventories* and click on *Insights Inventory*.
+
image:images/lab4/15-insightsinv.png[3000,3000]

. Click *Hosts* and click on the host *ic4.summit.example.com*.
+
image:images/lab4/16-insights-hosts.png[3000,3000]
...
+
image:images/lab4/17-insightsdataic4.png[1000,1000]
image:images/lab4/18-insights-issues.png[1000,1000]

+
. Navigate to *Projects* from the main menu on the left and find the *Insights Planner Sync* project. Then, click the Sync button. This will sync the Red Hat Insights information with Ansible Tower.
+
image:images/lab4/19-insights-sync.png[1000,1000]
. The sync should complete successfully. Now you're ready to remediate with Ansible Tower.
+
NOTE: Ansible Tower can also identify Red Hat Insights issues on managed hosts. All of the above projects and plans can be synced and updated automatically with Ansible Tower via the API or with scheduled runs.
...
+
image:images/lab4/20-jobtemplate.png[2000,2000]
...
+
image:images/lab4/21-new_job.png[3000,3000]
...
+
image:images/lab4/22-new-template-created.png[3000,3000]

. Run this job template by clicking the *rocket ship* for the template you created. This will launch the job and run the playbook you selected from the dropdown. If everything is successful, you will see a status of *Successful* in the DETAILS pane. The Ansible Tower log will also show zero failures on the right in the PLAY RECAP at the bottom of the log.
+
image:images/lab4/23-playbook_is_run.png[3000,3000]
...
+
image:images/lab4/24-results_in_satellite.png[3000,3000]
+
...
+
image:images/lab4/25-insightsplanner.png[300,300]
image:images/lab4/8-planner.png[1000,1000]

<<top>>

link:README.adoc#agenda[ Table of Contents ] | link:lab5.adoc[Lab 5: Automated Patching]
