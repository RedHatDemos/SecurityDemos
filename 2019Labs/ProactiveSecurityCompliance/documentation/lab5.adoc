...
. You can specify commands and files you don't want to access by creating a remove.conf file. Let's create a remove.conf file that does the following (INSERT DETAIL HERE)
+
...
. Now, let's run the Insights collection from the command line.
+
...
. Install and register this client to Red Hat Insights:
+
...
. Navigate to *Red Hat Insights → Overview*, where you could see all your registered systems, actions summary (highlighted by priority) as well as latest updates from Red Hat.
+
