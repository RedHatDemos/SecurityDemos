== (THIS LAB EXERCISE DOCUMENTATION STILL NEEDS TO BE UPDATED!!) Lab 3: How to use the control policy engine in Red Hat CloudForms and Red Hat Ansible Automation to enforce automated compliance with security policies
...
. If not already there, log into to the bastion host as *root* from your desktop system *replacing GUID with your lab's GUID*. Use the password *r3dh4t1!* for *root*.
...
. Log into the *lab3-vm1.example.com* host using its IP address *192.168.1.171*.
+
[source]
----
