...
WORKS!
----
NOTE: This step works even without a reboot because the wget is a command line binary and not a long term running process. Reboot is recommended to make sure all resident services are restarted and re-read their configureation (including the libraries they instantiate).
