...
----
	...
	[lab-user@workstation-GUID ~]$ scp ipsec2.example.com.p12 /root/oe-cert.conf root@ipsec2.example.com:/root/
...
as the new nodes. In a production setting, you could be able to populate new nodes
