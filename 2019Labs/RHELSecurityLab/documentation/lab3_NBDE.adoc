...
== Lab 3.6 Overview of the feature in Cockpit

This is not an actual exercise, but rather a presentation of NBDE in Cockpit. The process is fairly simple and straight forward. You do not need to do actual steps.

"*Storage*" in Cockpit provides a way to manage disks.

image:images/nbde_cockpit_storage.png[]

Accessing the "*Encryption*"" tab of a particular disk allows to manage it's encryption keys. If the disk is encrypted initially it shows only passphrase:

image:images/nbde_cockpit_encryption tab.png[]

Clicking on *+* button allows to add more keys. Key can be a passphrase or Tang keyserver. To add Tang keyserver key keyserver address must be provided together with existing disk passphrase:

image:images/nbde_cockpit_tang_filled.png[]

Before adding the Keyserver there is an option to verify the key hash:

image:images/nbde_cockpit_tang_confirm.png[]

After keyserver is added, both Disk passphrase and Keyserver are shown as keys for the disk:

image:images/nbde_cockpit_keys_result.png[]

This disk will be unlocked with the existing passphrase as well as with the Clevis policy.
