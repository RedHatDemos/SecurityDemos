...
Using the less command, you can jump to line 200 by typing 200g. When you do that, you should see these two lines at the top:

/etc/ssh/sshd_config$ LSPP
/etc/ssh/ssh_config$ LSPP
----
. What this means is that AIDE is monitoring these two files and is looking specifically for changes to content based on sha256 and sha512 hashes.  At line 80, you will find the definition for LSPP:
+
----
[root@servera ~]# less /etc/aide.conf

Using the less command, you can jump to line 80 by typing 80g. When you do that, you should see these lines at the top:
...
Using the less command, you can jump to line 26 by typing 26g. When you do that you should see the list of the parameters that are included. Type q to exit less, and then type the following commands.
...
[source]
[root@servera ~]$ aide --check
AIDE 0.16.1 found differences between database and filesystem!!
...
. Run *aide --check* again to verify that you have reverted back correctly.  It will show a change in the timestamp, but not to the content.
+
[source]
[root@servera ~]# aide --check
