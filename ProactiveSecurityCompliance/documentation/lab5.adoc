...
. Now let's fix this issue. Before we do that, let's go into our *lab5-vm1* by SSH or via the console button on the main *Lab Information* webpage. In this step, we'll use SSH. SSH into the *lab5-vm1* VM. First, SSH into your workstation VM as *lab-user* (using the password *r3dh4t1!*) and then from there, SSH into your *lab5-vm1* as root.
+
[source, text]
