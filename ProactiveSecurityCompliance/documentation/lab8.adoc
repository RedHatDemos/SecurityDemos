...
. If not already root, become *root* and then log into the *ic4.summit.example.com*.
+
[source]
----
[lab-user@workstation-GUID ~]$ sudo -i
...
+
IMPORTANT: It is possible that one of the nodes will fail during remediation. Simply re-launch the job by clicking on the *Rocket*.
+
