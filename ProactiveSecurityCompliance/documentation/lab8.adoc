...
For this part of the lab exercise, we will the payload injection vulnerability using an Ansible playbook vs doing it manually like we did in the previous lab exercise. We will execute these steps on the *ic4.summit.exmaple.com VM.

. In this particular case of the payload injection security issue, an Ansible Playbook is available for us.
+
