...
. Now, let's power on our hosts. On the CloudForms appliance (cfme-GUID.rhpds.opentlc.com) log in with *admin* as the user name and *r3dh4t1!* as the password (if not already logged in). On the left side menu, navigate to *Compute → Infrastructure → Virtual Machines*.
+
